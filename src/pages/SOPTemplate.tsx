      ...
      const section = (title: string, content: string) =>
        content
          ? `<div style="margin-bottom:14px;">
              ...
              <p style="font-size:12px;line-height:1.8;white-space:pre-wrap;margin:0;">${esc(content)}</p>
            </div>`
          : "";
      ...
      const html = `
        <div style="direction:rtl;font-family:'Cairo',sans-serif;padding:20px;max-width:700px;">
          <div style="display:flex;justify-content:space-between;align-items:center;border:2px solid #0ea5e9;border-radius:8px;padding:12px 16px;margin-bottom:16px;">
            <div>
              <h1 style="font-size:18px;font-weight:bold;color:#0284c7;margin:0;">${esc(form.title)}</h1>
            ...
            <div style="text-align:left;font-size:11px;color:#334155;">
              ${form.docNumber ? `<div>رقم الوثيقة: <strong>${esc(form.docNumber)}</strong></div>` : ""}
              ${form.revision ? `<div>رقم المراجعة: <strong>${esc(form.revision)}</strong></div>` : ""}
              ${form.effectiveDate ? `<div>تاريخ السريان: <strong>${esc(form.effectiveDate)}</strong></div>` : ""}
            </div>
          </div>
          <table style="width:100%;border-collapse:collapse;margin-bottom:16px;font-size:11px;">
            <tr>
              <td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;width:25%;">القسم</td>
              <td style="padding:6px 10px;border:1px solid #d1d5db;width:25%;">${esc(form.department)}</td>
              <td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;width:25%;">التصنيف</td>
              <td style="padding:6px 10px;border:1px solid #d1d5db;width:25%;">${esc(form.category)}</td>
            </tr>
            <tr>
              <td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;">إعداد</td>
              <td style="padding:6px 10px;border:1px solid #d1d5db;">${esc(form.preparedBy)}</td>
              <td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;">اعتماد</td>
              <td style="padding:6px 10px;border:1px solid #d1d5db;">${esc(form.approvedBy)}</td>
            </tr>
          </table>
