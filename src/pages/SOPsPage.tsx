      ...
      const stepsRows = steps
        ...
        .map((s: any, i: number) => `
          <tr>
            ...
            <td style="padding:8px;border:1px solid #d1d5db;">${esc(s.description)}</td>
            <td style="padding:8px;border:1px solid #d1d5db;text-align:center;">${esc(s.responsible)}</td>
            <td style="padding:8px;border:1px solid #d1d5db;">${esc(s.notes)}</td>
          </tr>`)
        .join("");

      const section = (title: string, content: string) =>
        content ? `<div style="margin-bottom:14px;"><h3 style="font-size:14px;font-weight:bold;color:#0284c7;margin-bottom:4px;border-bottom:2px solid #0ea5e9;padding-bottom:2px;">${title}</h3><p style="font-size:12px;line-height:1.8;white-space:pre-wrap;margin:0;">${esc(content)}</p></div>` : "";
      ...
      const html = `
        <div style="direction:rtl;font-family:'Cairo',sans-serif;padding:20px;max-width:700px;">
          <div style="display:flex;justify-content:space-between;align-items:center;border:2px solid #0ea5e9;border-radius:8px;padding:12px 16px;margin-bottom:16px;">
            <div><h1 style="font-size:18px;font-weight:bold;color:#0284c7;margin:0;">${esc(data.title)}</h1><p style="font-size:11px;color:#64748b;margin:2px 0 0;">إجراء تشغيل قياسي - SOP</p></div>
            <div style="text-align:left;font-size:11px;color:#334155;">
              ${data.doc_number ? `<div>رقم الوثيقة: <strong>${esc(data.doc_number)}</strong></div>` : ""}
              ${data.revision ? `<div>رقم المراجعة: <strong>${esc(data.revision)}</strong></div>` : ""}
              ${data.effective_date ? `<div>تاريخ السريان: <strong>${esc(data.effective_date)}</strong></div>` : ""}
            </div>
          </div>
          <table style="width:100%;border-collapse:collapse;margin-bottom:16px;font-size:11px;">
            <tr><td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;width:25%;">القسم</td><td style="padding:6px 10px;border:1px solid #d1d5db;width:25%;">${esc(data.department)}</td><td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;width:25%;">التصنيف</td><td style="padding:6px 10px;border:1px solid #d1d5db;width:25%;">${esc(data.category)}</td></tr>
            <tr><td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;">إعداد</td><td style="padding:6px 10px;border:1px solid #d1d5db;">${esc(data.prepared_by)}</td><td style="padding:6px 10px;border:1px solid #d1d5db;background:#f0f9ff;font-weight:bold;">اعتماد</td><td style="padding:6px 10px;border:1px solid #d1d5db;">${esc(data.approved_by)}</td></tr>
          </table>
